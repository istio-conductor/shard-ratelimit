...
func ExamplePacketConn_servingOneShotMulticastDNS() {
	...
	if err := p.SetControlMessage(FlagDst, true); err != nil {
		log.Fatal(err)
	}
...
func ExamplePacketConn_tracingIPPacketRoute() {
	...
	if err := p.SetControlMessage(FlagTTL|FlagSrc|FlagDst|FlagInterface, true); err != nil {
		log.Fatal(err)
	}
	wm := icmp.Message{
...
func ExampleRawConn_advertisingOSPFHello() {
	...
	case "darwin", "ios", "linux":
		cm = &ControlMessage{IfIndex: en0.Index}
	default:
