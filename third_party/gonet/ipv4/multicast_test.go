...
func TestPacketConnReadWriteMulticastUDP(t *testing.T) {
	...
	for _, tt := range packetConnReadWriteMulticastUDPTests {
		t.Run(fmt.Sprintf("addr=%s/grp=%s/src=%s", tt.addr, tt.grp, tt.src), func(t *testing.T) {
			...
			cf := FlagTTL | FlagDst | FlagInterface
		...
		t.Run(fmt.Sprintf("grp=%s/src=%s", tt.grp, tt.src), func(t *testing.T) {
			...
			for i, toggle := range []bool{true, false, true} {
				...
				if n, _, _, err := p.ReadFrom(rb); err != nil {
					...
					switch {
					case m.Type == ICMPTypeEchoReply && m.Code == 0: // net.inet.icmp.bmcastecho=1
					case m.Type == ICMPTypeEcho && m.Code == 0: // net.inet.icmp.bmcastecho=0
					default:
						t.Fatalf("got type=%v, code=%v; want type=%v, code=%v", m.Type, m.Code, ICMPTypeEchoReply, 0)
					}
				}
			}
		})
	...
	for _, tt := range rawConnReadWriteMulticastICMPTests {
		...
		for i, toggle := range []bool{true, false, true} {
			...
			if rh, b, _, err := r.ReadFrom(rb); err != nil {
				...
				switch {
				case (rh.Dst.IsLoopback() || rh.Dst.IsLinkLocalUnicast() || rh.Dst.IsGlobalUnicast()) && m.Type == ICMPTypeEchoReply && m.Code == 0: // net.inet.icmp.bmcastecho=1
				case rh.Dst.IsMulticast() && m.Type == ICMPTypeEcho && m.Code == 0: // net.inet.icmp.bmcastecho=0
				default:
					t.Fatalf("got type=%v, code=%v; want type=%v, code=%v", m.Type, m.Code, ICMPTypeEchoReply, 0)
				}
			}
