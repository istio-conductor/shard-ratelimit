...
func testVM(t *testing.T, filter []Instruction) (virtualMachine, func(), error) {
	goVM, err := NewVM(filter)
	if err != nil {
...
func testOSVM(t *testing.T, filter []Instruction) (virtualMachine, func()) {
	l, err := nettest.NewLocalPacketListener("udp")
	if err != nil {
		t.Fatalf("failed to open OS VM UDP listener: %v", err)
	}

	prog, err := Assemble(filter)
	if err != nil {
