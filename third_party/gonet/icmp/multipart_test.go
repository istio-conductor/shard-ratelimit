...
func TestMarshalAndParseMultipartMessage(t *testing.T) {
	fn := func(t *testing.T, proto int, tm Message) error {
		...
		switch m.Type {
		case ipv4.ICMPTypeExtendedEchoRequest, ipv6.ICMPTypeExtendedEchoRequest:
			got, want := m.Body.(*ExtendedEchoRequest), tm.Body.(*ExtendedEchoRequest)
		...
		case ipv4.ICMPTypeDestinationUnreachable:
			got, want := m.Body.(*DstUnreach), tm.Body.(*DstUnreach)
			if !reflect.DeepEqual(got.Extensions, want.Extensions) {
				return errors.New(dumpExtensions(got.Extensions, want.Extensions))
			}
			if len(got.Data) != 128 {
		...
		case ipv4.ICMPTypeTimeExceeded:
			got, want := m.Body.(*TimeExceeded), tm.Body.(*TimeExceeded)
			if !reflect.DeepEqual(got.Extensions, want.Extensions) {
				return errors.New(dumpExtensions(got.Extensions, want.Extensions))
			}
			if len(got.Data) != 128 {
		...
		case ipv4.ICMPTypeParameterProblem:
			got, want := m.Body.(*ParamProb), tm.Body.(*ParamProb)
			if !reflect.DeepEqual(got.Extensions, want.Extensions) {
				return errors.New(dumpExtensions(got.Extensions, want.Extensions))
			}
			if len(got.Data) != 128 {
