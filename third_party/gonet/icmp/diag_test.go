...
func doDiag(dt diagTest, seq int) error {
	c, err := ListenPacket(dt.network, dt.address)
	if err != nil {
		return err
	}
	defer c.Close()
	...
	rm, err := ParseMessage(dt.protocol, rb[:n])
	if err != nil {
		return err
	}
	switch {
...
func googleAddr(c *PacketConn, protocol int) (net.Addr, error) {
...
func TestConcurrentNonPrivilegedListenPacket(t *testing.T) {
	...
	for i := 0; i < N; i++ {
		go func() {
			defer wg.Done()
			c, err := ListenPacket(network, address)
			if err != nil {
				t.Error(err)
				return
			}
	...
	nonPrivOnce.Do(func() {
		...
		case "linux":
			for _, t := range []struct{ network, address string }{
				...
				c, err := ListenPacket(t.network, t.address)
				if err != nil {
