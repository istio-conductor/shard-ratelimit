...
func BenchmarkReadWriteUnicast(b *testing.B) {
	...
	b.Run("IPv6UDP", func(b *testing.B) {
		p := NewPacketConn(c)
		cf := FlagTrafficClass | FlagHopLimit | FlagSrc | FlagDst | FlagInterface | FlagPathMTU
...
func testPacketConnConcurrentReadWriteUnicast(t *testing.T, p *PacketConn, data []byte, dst net.Addr, batch bool) {
	t.Helper()
