...
func ExamplePacketConn_servingOneShotMulticastDNS() {
	...
	if err := p.SetControlMessage(FlagDst|FlagInterface, true); err != nil {
		log.Fatal(err)
	}

	var wcm ControlMessage
	b := make([]byte, 1500)
	for {
...
func ExamplePacketConn_tracingIPPacketRoute() {
	...
	if err := p.SetControlMessage(FlagHopLimit|FlagSrc|FlagDst|FlagInterface, true); err != nil {
		log.Fatal(err)
	}
	wm := icmp.Message{
